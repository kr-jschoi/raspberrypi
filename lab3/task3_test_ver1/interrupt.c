...
ISR(TIMER1_COMPB_vect) {
	...
	else if (send_select == SEND_COMPLETE) { send_complete = 1; } // send complete
}
...
ISR(PCINT2_vect) {
	if (receive_select == PREAMBLE_FLAG) { // receive preamble
		...
		if (j == (PREAMBLE_SIZE * 8)){
			if (preamble_check(received_preamble) == TRUE) { j = 0; receive_select = CRC_FLAG;} // Preamble complete
		j++;
		}
		else if(j>8)
		{
	...
	else if (receive_select == SIZE_FLAG) { // receive CRC
	...
	else if (receive_select == LAYER3) {
		ID_check(send_frame, receive_frame);
	}
	else if (receive_select == RECEIVE_COMPLETE) { receive_complete = 1; } // recieve complete
}
